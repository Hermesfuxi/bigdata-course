...
/**
 * 活动事件表的用户统计（UV - count(distinct uid))与用户浏览统计（PV - count(udi)）：BloomFilterState 实现
 * 将 BloomFilter 放入 state 中，支持故障恢复
 */
public class DistinctCountByHLLState {
    public static void main(String[] args) throws Exception {
        ...
        keyedStream.process(new KeyedProcessFunction<Tuple2<String, String>, Tuple3<String, String, String>, Tuple4<String, String, Integer, Integer>>() {
            ...
            public void open(Configuration parameters) throws Exception {
                ...
                ValueStateDescriptor<Byte[]> hllDescriptor = new ValueStateDescriptor<>("hllDescriptor", TypeInformation.of(new TypeHint<Byte[]>(){}));
                hllState = getRuntimeContext().getState(hllDescriptor);
            ...
            public void processElement(Tuple3<String, String, String> value, Context ctx, Collector<Tuple4<String, String, Integer, Integer>> out) throws Exception {
                ...
                HLL hll = null;
                Byte[] bytes = hllState.value();
                if(bytes == null){
                    hll = new HLL(14, 5);
                }else {
                    hll = HLL.fromBytes(ArrayUtils.toPrimitive(bytes));
                }
                hll.addRaw(IDUtils.getMD5(uid));
                long cardinality = hll.cardinality();
                hllState.update(ArrayUtils.toObject(hll.toBytes()));

                //输出
                out.collect(Tuple4.of(value.f1, value.f2, (int) cardinality, uidCount));
            }
        }).print();
